    ...
    public static function loadRoutes()
    {
        ...
        foreach ($types as $type) {
            Route::middleware(['web','auth', 'splade', 'verified'])
                ...
                Route::get('/edit/{item}', function (\TomatoPHP\TomatoCategory\Models\Type $item) use ($type){
                    return (new self)->edit($item, $type->for, $type->type, $type->label);
                })->name('edit');
                Route::delete('/delete/{item}', function (\TomatoPHP\TomatoCategory\Models\Type $item) use ($type){
